...
[![Framework](https://img.shields.io/badge/framework-Mbed%20OS%206-green)](https://os.mbed.com/docs/mbed-os/v6.16/)
[![License](https://img.shields.io/badge/license-MIT-orange)](LICENSE)

A complete embedded closed-loop system that acquires breath alcohol signals from an MQ-3 sensor, processes them through a trimmed-mean filter and a 7-state finite-state machine on the STM32L432KC, and delivers multi-modal feedback through an OLED display, dual-colour LEDs, an active buzzer, and BLE wireless telemetry.
...
| **Safe** | Filtered ADC < W threshold | Green LED on, buzzer off |
...
| **Cooldown** | ADC < D − 200 (×3 consecutive) | Red/green alternate, 10 s guard before re-evaluation |
...
State transitions use consecutive-sample counters for both entry and release, plus release hysteresis, eliminating false triggers from single-sample noise.
...
- [Mbed Studio](https://os.mbed.com/studio/) or [Mbed CLI](https://os.mbed.com/docs/mbed-os/v6.16/build-tools/mbed-cli.html)
...
Serial output at 115200 baud will show status changes and BLE telemetry frames.
...
3. Bring a weak alcohol stimulus (e.g. alcohol wipe at 15–20 cm) near the MQ-3 sensor.
   - System enters **Warning** — red LED blinks, buzzer beeps intermittently.
...
All report figures are generated programmatically with Python 3 and Matplotlib:
...
This produces three figures in SVG, PDF, and PNG formats:
- `fig1_system_architecture_cooltone` — System architecture block diagram
- `state_machine_cooltone` — 7-state finite-state machine diagram
- `fig2_system_characterization_cooltone` — Dual-panel ADC characterization (simulated data; replace with serial log for final submission)

---

## Report

The LaTeX report source is in [`report/MS_individual_project_report_template.tex`](report/MS_individual_project_report_template.tex). Compile with XeLaTeX:

```bash
cd report
xelatex MS_individual_project_report_template.tex
xelatex MS_individual_project_report_template.tex
```

Requires the following fonts: Arial, PingFang SC, Menlo (macOS default). On other platforms, the template falls back to TeX Gyre Heros, STSong, and Latin Modern Mono.

---

## Project Context

- **Course**: Microelectronic Systems (2025–26 Semester 2), UESTC × UoG
- **Author**: Yue Donghan (岳东翰) — UESTC ID: 2025190907024, UoG ID: 3180881
- **Objective**: Demonstrate a complete Sensor → MCU → Controller closed loop on the STM32L432KC platform
